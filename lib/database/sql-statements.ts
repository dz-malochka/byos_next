...
CREATE POLICY plugin_settings_capability_select_policy ON plugin_settings
	...
	"0022_playlist_fk_on_delete_set_null": {
		title: "Set devices.playlist_id FK to ON DELETE SET NULL",
		description:
			"Recreates the devices_playlist_id_fkey constraint with ON DELETE SET NULL so deleting a playlist no longer fails with a foreign key violation when devices still reference it. Devices lose their playlist association (playlist_id -> NULL) instead of blocking the delete. Idempotent (drops the constraint if present before recreating).",
		sql: `-- The original constraint (migration 0000) was created without an ON DELETE
